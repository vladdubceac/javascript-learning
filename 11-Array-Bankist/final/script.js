...
console.log('vlad'.at(0));
console.log('vlad'.at(-1));
*/
...
console.log(dogsAges1);
const averageAge1 = calcAverageHumanAge(dogsAges1);
console.log(averageAge1);

console.log(dogsAges2);
const averageAge2 = calcAverageHumanAge(dogsAges2);
...
console.log(convertTitleCase('and here is another tile with an EXAMPLE'));
*/
...
let output = `Sarah's dog is eating `;
...
// 4.
const testOutput1 = ownersEatTooMuch.reduce((output, owner, i, arr) => {
  output += owner;
  if (arr.length > 1 && i < arr.length - 1) {
    output += ' and ';
  }
  if (i === arr.length - 1) {
    output += `'s dogs eat too much!`;
  }
  return output;
}, '');
...
const testOutput2 = ownersEatTooLittle.reduce((output, owner, i, arr) => {
  output += owner;
  if (arr.length > 1 && i < arr.length - 1) {
    output += ' and ';
  }
  if (i === arr.length - 1) {
    output += `'s dogs eat too little!`;
  }
  return output;
}, '');
...
// 7.
const dogsEatingOkayAmount = dogs.filter(checkEatingOkay);
console.log(dogsEatingOkayAmount);
// 8.
// let shallowCopy = Array.from(dogs);
// shallowCopy.sort((dog1, dog2) => dog1.recommendedFood - dog2.recommendedFood);
